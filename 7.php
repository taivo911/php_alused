...
<div class="container">
    <div class="row">
        <div class="col-md-6">
            <h3>Tydrukud</h3>
            <?php
            $tydrukud = array('kristiina', 'kati', 'elika', 'kristi', 'carmen', 'krista', 'ester', 'esta');
            //sorteerimine
            sort($tydrukud);
...
//loendan autode koguse
            echo count($autod);

//vrdlen autode ja vin koodide koguseid
            echo count($autod) == count($vin) ? ", List on sama pikkusega <br>" : ", List ei ole sama pikkusega <br>";

            $audisid = 0;
            $toyotasid = 0;
            foreach($autod as $auto){
                if ($auto == 'Audi') {$audisid++;}
                else if ($auto == 'Toyota') {$toyotasid++;}
            }
            echo "Audisid on ".$audisid."<br>";
            echo "Toyotasid on ".$toyotasid."<br>";

//vin koodide pikkuse vordlus
            echo "Vin lyhem kui 17 t2hem2rki <br>";
            for(
                $v = 0;
                $v<=count($vin)-1;
                $v ++) {
...
echo "Nimekiri kustutatud firmaga: ".$break;
if (isset($_GET['firma']) or isset($_GET['firma'])) {
    if(strlen($_GET['firma']) and strlen($_GET['firma'])) {
        for ($fir = 0; $fir<=count($firmad)-1; $fir++) {
            if ($firma == $firmad[$fir]) {
                continue;
            } else {
                echo $firmad[$fir].$break;
            }
        }
...
?>
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            $max_pikkus = $riigid[0];
            for ($i = 1; $i < count($riigid); $i++) {
                strlen($riigid[$i]) > $max_pikkus ? $max_pikkus = strlen($riigid[$i]) : false;
            }
            echo "Kõige pikema nimega riik ".$max_pikkus."<br>";
            ?>
        </div>
        <div class="col-md-6">
            <h3>Hiina nimed</h3>
            ...
            sort($hiina_nimed);
            echo "Esimene - ".$hiina_nimed[0]."      Viimane - ".$hiina_nimed[count($hiina_nimed)-1]."<br>";
            ?>

            <h3>Kuuluvus massiivi</h3>
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
                <p>Kontrolli kuuluvust massiivi</p>
                <input type="text" name="massiiv">
                <button type="submit" name="search">Otsi</button>
            </form>
            <?php
            //Google
            ...
            $massiiv = '';
            in_array($_POST['massiiv'], $google) ? $massiiv = "Nimi on olemas!"
            : $massiiv = "Nimi puudub!";
            echo "<div class='alert alert-primary' role='alert'>".$massiiv."</div>";
            }
            ?>
        </div>
    </div>
    <div class="row">
        ...
        <div class="col-md-2">
            <?php
            echo "Massiivi kolmas pilt <img src='img/$pildid[2]' width='70px'>";
            ?>
        </div>
        <div class="col-md-2">
            <?php
            echo "Massiivi esimene pilt <img src='img/$pildid[0]' width='70px'>";
            ?>
        </div>
        <div class="col-md-2">
            <?php
            echo "Massiivi teine pilt <img src='img/$pildid[1]' width='70px'>";
            ?>
        </div>
        <div class="col-md-2">
            <?php
            echo "Massiivi neljas pilt <img src='img/$pildid[3]' width='70px'>";
            ?>
        </div>
        <div class="col-md-2">
            <?php
            echo "Massiivi viies pilt <img src='img/$pildid[4]' width='70px'>";
            ?>
        </div>
        <div class="col-md-2">
            <?php
            echo "Massiivi kuues pilt <img src='img/$pildid[5]' width='70px'>";
            ?>
        </div>
    </div>
</div>
